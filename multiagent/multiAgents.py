...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # set the distance to the closest food to very large number
        ...
        # get successor's ghost position
        ghosts_location_list = successorGameState.getGhostPositions()

        # set the distance to the closest ghost to very large number
        ...
        # if the distance between successor and the closest ghost is less than 2 unit return a negative evaluation value
        if closest_ghost_distance < 2:
            return - 999999999

        '''
        if the evaluation score is larger than 0 (2 unit distance away from the closest ghost), 
        return the current score plus the closest ghost distance divided by closest food distance
        since the bigger the closest ghost distance and the lower the closest food distance the successor have
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax_decision(self, gameState, cur_depth, agentIndex):
        ...
        param agentIndex: agent ID, 0 is pacman, all others number are ghosts
        return: the best action based on minimax decision
        """

        # get legal actions list by calling the provided getLegalActions function
        legal_actions = gameState.getLegalActions(self.index)

        max_score = -999999999
        temp_action_list = []

        # call the min value which is recursively call max_value and min_value and eventually return the action with
    ...
    def max_value(self, gameState, cur_depth):
        ...
        param gameState: current game state
        param cur_depth: current depth in the tree
        param agentIndex: agent ID, should always be 0 in this function as max value is used for pacman
        ...
        # reach the depth limitation, return the evaluation value of current game state
    ...
    def min_value(self, gameState, cur_depth, agentIndex):
        ...
        # If the current agent index is not the last agent index.
        ...
        # if the current agent index is the last agent index
    ...
    # terminal test which test if the game is ended by checking isWin and isLost functions
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        # call the alpha_beta_pruning_decision function below and save the returned action and return it Since the
        # alpha_beta_pruning_decision function handle the max value function at depth 0 for pacman(agentIndex = 0) so
        # that, the parameter passed in are depth = 1  and agentIndex = 1
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # call the expectimax decision function below and save the returned action and return it
        # Since the expectimax decision function handle the max value function at depth 0 for pacman(agentIndex = 0)
        # so that, the parameter passed in are depth = 1  and agentIndex = 1
    ...
    def max_value(self, gameState, cur_depth):
        ...
        # get legal actions list for pacman by calling the provided getLegalActions function
        action_list = gameState.getLegalActions(pacman_index)
    ...
    def expect_value(self, gameState, cur_depth, agentIndex):
        ...
        # get legal actions list for ghost by calling the provided getLegalActions function
        action_list = gameState.getLegalActions(agentIndex)
        expect = 9999999
        ...
        # if it is not the last agent, means the next agent is ghost we should call expect_value function, add agent
        # index by 1 and also append to expect value into the expect_value_list
        if agentIndex != last_agent_index:
            for each_legal_action in action_list:
        ...
        # we should call the max_value function and append to expect value into the expect_value_list
        else:
            for each_legal_action in action_list:
                temp_value = self.max_value(gameState.generateSuccessor(agentIndex, each_legal_action), cur_depth)
                expect_value_list.append(temp_value)

        # calculate the average expect valuing the list
...
def betterEvaluationFunction(currentGameState):
    ...
    if currentGameState.getScore() > 0:
        ...
        # if there is no scared time, the higher the closest ghost distance, the lower the closest food distance and the
        # closest capsule distance the better
        if total_scared_time == 0:
            if closest_ghost_distance > closest_food_distance:
                # add the closest capsule distance to the denominator
                return currentGameState.getScore() + (
                        closest_ghost_distance / (closest_food_distance + closest_capsule_distance))
            else:
                return currentGameState.getScore()

        # if there is scared time, pacman should looking for ghost instead of food
        # so that, the higher the distance to the closest food and the lower the distance to the closest ghost
        # the better
        else:
